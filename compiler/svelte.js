...
var voidElementNames = /^(?:area|base|br|col|command|doctype|embed|hr|img|input|keygen|link|meta|param|source|track|wbr)$/;
...
function validate ( parsed, source, { onerror, onwarn, name, filename } ) {
...
function process ( parsed ) {
	return transformCss( parsed.css.content.styles, parsed.hash );
}
...
function addElementAttributes ( generator, node, local ) {
	node.attributes.forEach( attribute => {
		if ( attribute.type === 'Attribute' ) {
			let metadata = local.namespace ? null : lookup[ attribute.name ];
...
class DomGenerator extends Generator {
	constructor ( parsed, source, names, visitors$$1 ) {
		super( parsed, source, names, visitors$$1 );
		this.renderers = [];
		this.uses = {};
	}
...
function dom ( parsed, source, options, names ) {
	...
	if ( parsed.css && options.css !== false ) {
		...
		builders.main.addBlock( deindent`
			...
			function addCss () {
				...
				style.textContent = ${JSON.stringify( process( parsed ) )};
	...
	if ( sharedPath ) {
		...
		builders.main.addBlock( dispatchObservers.toString() );
...
function ssr ( parsed, source, options, names ) {
	...
	const generator = new SsrGenerator( parsed, source, names, visitors$1 );
